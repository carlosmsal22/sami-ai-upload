...
def show_main_app():
    ...
    # Do NOT call st.set_page_config here again
    ...
    # --- Sidebar Definition ---
    with st.sidebar:
        # Use page_link for navigation if using multi-page apps later,
        # otherwise buttons are fine for setting state in a single-page app.
        st.page_link("Home.py", label="Home", icon="🏠") # Link to the main script
        st.markdown("---")
        st.subheader("Analysis Modules")

        module_buttons = {
            "CBC Conjoint": "CBC",
            "CrossTabs Analyzer Phase1": "CrossTabs1",
            "Enhanced CrossTabs Analyzer": "CrossTabs2",
            "Executive Insight Generator old": "ExecOld",
            "LCA Module": "LCA",
            "MaxDiff Module": "MaxDiff",
            "OLD CrossTabs Analyzer": "OldCrossTabs1",
            "OLD CrossTabs Step2": "OldCrossTabs2",
            "Persona From PPTX": "PersonaPPTX",
            "Persona Generator": "PersonaGen",
            "SAMI Analyzer": "SAMI",
            "SEM Module": "SEM",
            "Text Analytics": "Text",
            "TURF Module": "TURF"
        ...
        for label, key in module_buttons.items():
            if st.button(label, key=f"btn_{key}"):
                ...
                st.write(f"DEBUGGING: Sidebar button '{label}' clicked, current_module set to: {key}") # Debug print
                # Optional: Add a rerun if clicking the button should immediately change the main view
                # st.rerun()

    # --- Main Area Content (Placeholder for Module Logic) ---
    if 'current_module' in st.session_state:
        st.write(f"DEBUGGING: Displaying content for module: {st.session_state.current_module}") # Debug print
        # Add your logic here to display content based on st.session_state.current_module
        st.header(f"Module: {st.session_state.current_module}")
        st.info("Module content goes here...")
    else:
        st.write("DEBUGGING: No module selected yet.") # Debug print
        # Keep the welcome message visible if no module is chosen

    st.write("DEBUGGING: Exiting show_main_app()") # Debug print
...
if 'start_app' in st.query_params and st.query_params['start_app'] == 'true':
    ...
    st.write(f"DEBUGGING: view_state set to 'app' based on query param.") # Debug print
    # It will automatically rerun after setting query params, no need for explicit rerun here
...
# --- Temporarily Disable CSS Hiding ---
# Comment out this section to ensure CSS isn't causing the blank page issue
# hide_streamlit_style = """
#             <style>
#             #MainMenu {visibility: hidden;}
#             footer {visibility: hidden;}
#             /* header[data-testid="stHeader"] {visibility: hidden;} /* Might hide too much */
#             /* Target the specific Streamlit header bar more carefully if needed later */
#             div[data-testid="stToolbar"] { visibility: hidden !important; } /* Try hiding toolbar */
#             header[data-testid="stHeader"] { background: none; border: none; } /* Make header transparent */
#             </style>
#             """
# Only hide elements when not in the main app view
# if st.session_state.get('view_state', 'image') != 'app':
#     st.markdown(hide_streamlit_style, unsafe_allow_html=True)
#     st.write("DEBUGGING: Applying CSS to hide elements.") # Debug print
# else:
#     st.write("DEBUGGING: Not applying CSS hiding (in app view).") # Debug print
