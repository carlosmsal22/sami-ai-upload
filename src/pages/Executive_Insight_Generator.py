...
if uploaded_file:
    ...
    banner_options = [
        "Total", "Residential Units Managed", "Job Role", "Job Title",
        "Change in Living Expectations", "Track Utility Expenses",
        "Percent of Rent Delinquent", "Security Deposit Return Method",
        "Staff Turnover Rate", "Digital vs Paper Rent Payments"
    ]
        ...
        while row < len(df):
            if "Table Title" in str(df.iloc[row, 1]):
                try:
                    ...
                    st.warning(f"⚠️ Error parsing table at row {row}: {e}")
                    row += 1
            else:
                row += 1
        return tables
    ...
    def generate_insights(df_table):
        ...
        for _, row in rows.iterrows():
            try:
                ...
                summary_lines.append(f"• **{segments[high_idx]}** leads in *{metric.lower()}* at {high_val}, while **{segments[low_idx]}** is lowest at {low_val}.")
            except:
                continue
    ...
    # Parse tables and show options
    tables = parse_tables(df)
    if not tables:
        st.error("❌ No tables detected. Please verify the file format.")
        st.stop()

    table_titles = [f"Table {i+1}: {t[0][:60]}" for i, t in enumerate(tables)]
    selected_idx = st.sidebar.selectbox("🧾 Select Table to Preview", options=range(len(tables)), format_func=lambda x: table_titles[x])
    ...
    if insights:
        ...
        st.info("No insights could be generated from this table.")
    ...
    # --- Export options ---
    if st.sidebar.button("📄 Download DOCX Report"):
        doc = Document()
        ...
        for title, df in tables:
            ...
            doc.add_paragraph("Further strategic implications can be drawn from the segment behaviors and differences above.")
        with tempfile.NamedTemporaryFile(delete=False, suffix=".docx") as tmp:
            doc.save(tmp.name)
            with open(tmp.name, "rb") as file:
                st.sidebar.download_button("⬇️ Download Word File", file.read(), file_name="Executive_Insights_Report.docx")
    ...
    if st.sidebar.button("📄 Download PDF Report"):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            pdf.output(tmp.name)
            with open(tmp.name, "rb") as file:
                st.sidebar.download_button("⬇️ Download PDF File", file.read(), file_name="Executive_Insights_Report.pdf")
else:
    st.warning("📁 Please upload a crosstab Excel file to begin.")
